...
def process(args, log):
    ...
    with HashedSVIO(args.input_status, title_starter="") as FH_in:
        ...
        if len(set(loci_in_bed) - loci_in_status) > 0:
            msg = "The following loci are defined in targets but are missing from status file: {}".format(set(loci_in_status) - loci_in_status)
            log.error(msg)
            raise Exception(msg)
            ...
            for record in FH_in:
                for locus in loci_in_bed:
                    ...
                    FH_out.write({
                        ...
                        "locus_position": locus if args.loci_by_id else id_by_name[locus],
                        "method_id": "model",
                        "key": "status",
                        "value": record[locus],
                        "type": "str"
...
if __name__ == "__main__":
    ...
    parser.add_argument('-c', '--loci-by-id', action='store_true', help='Use this option if the titles in input-status corrspond to the loci IDs (format: chr:start-end with start 0-based) and not the names defined in input-target.')
