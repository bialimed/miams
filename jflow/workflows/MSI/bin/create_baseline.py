...
def process(args, log):
    ...
    with open(args.input_aln) as f:
        for line in f:
            line = line.rstrip()
            # Mpileup
            log.info("Start samptools mpileup")
            mpileup_output = os.path.join(working_directory, os.path.basename(line).rsplit(".", 1)[0] + ".mpileup.txt")
            ...
            filtered_mpileup_output = os.path.join(working_directory, os.path.basename(line).rsplit(".", 1)[0] + ".filtered_mpileup.txt")
            fout = open(filtered_mpileup_output,'w')
            cmd = [
                "awk",
                '{if($4 >= 6) print $0}',
                mpileup_output
            ...
            varscan_output = os.path.join(working_directory, os.path.basename(line).rsplit(".", 1)[0] + ".varscan.txt")
            cmd = [
                "java",
            ...
            tmp_analyzer_filename = os.path.basename(args.output_baseline).rsplit(".", 1)[0] + ".msi.txt"
            tmp_output_analyzer = os.path.join(working_directory,  os.path.basename(line).rsplit(".", 1)[0] + ".msi.txt")
            cmd = [
                msi_path,
                "analyzer",
                varscan_output,
...
if __name__ == "__main__":
    ...
    group_input.add_argument('-a', '--input-aln', required=True, help="The list of alignment file to evaluate (format: list). All BAMs must be ordered by coordinates and indexed.")
