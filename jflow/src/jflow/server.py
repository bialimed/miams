...
class JFlowServer (object):
    ...
    def get_available_workflows(self, **kwargs):
        ...
        for instance in wf_instances:
            ...
            for param in instance.get_parameters():
                ...
                if param.rules is not None:
                    hash_param["rules"] = param.rules
                ...
                if type(param.type) == MultipleParameters:
                    ...
                    for sub_param in param.sub_parameters:
                        ...
                        if sub_param.rules is not None:
                            hash_param["sub_parameters"][-1]["rules"] = sub_param.rules
    ...
    def _webify_outputs(self, web_path, path):
        ...
        else:  # Remove directories from the list
            return None
        # TODO: show output directories and add the possibility to browse them

    @cherrypy.expose
    @jsonify
    def get_workflow_outputs(self, **kwargs):
        on_disk_outputs, on_web_outputs = self.wfmanager.get_workflow_outputs(kwargs["workflow_id"]), OrderedDict()
        for cpt_name in list(on_disk_outputs.keys()):
            ...
            for outf in on_disk_outputs[cpt_name]:
                ...
                if on_web_outputs[cpt_name][outf] is None:  # Remove directories from the list
                    del on_web_outputs[cpt_name][outf]
    ...
    @staticmethod
    def sort_files(all_files, parent_directory):
        dirs = []
        files = []
        for f in all_files:
            if f[0] != ".":
                if os.path.isdir(parent_directory + "/" + f):
                    dirs.append(f)
                else:
                    files.append(f)
    ...
    @cherrypy.expose
    def get_user_files(self, **kwargs):
        checkbox = "<input type='checkbox' style='display: None;' />" if "multiSelect" in kwargs and kwargs["multiSelect"] else ""
        r=['<ul class="jqueryFileTree" style="display: none;">']
        try:
            r=['<ul class="jqueryFileTree" style="display: none;">']
            d=kwargs["dir"] if kwargs["dir"] != "/" else self.get_browse_root_dir()
            for f in self.sort_files(os.listdir(d), d):
                ...
                if os.path.isdir(ff):
                    ...
                    r.append(('<li class="file ext_%s">' + str(checkbox) + '<a rel="%s">%s</a></li>') % (e,ff,f))
            r.append('</ul>')
        except Exception as e:
