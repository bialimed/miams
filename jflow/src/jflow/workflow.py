...
#                         break
        ...
        for parameter in parameters:
            ...
            # Other parameters:
                ...
                if isinstance(parameter, InputFileList) and parameter.name in user_params:
                    ...
                    if isinstance(user_params[parameter.name],list) and len(set(user_params[parameter.name])) < len(user_params[parameter.name]):
                        raise RuleException("Error for parameter '" + parameter.name + "': some files are duplicated"+user_params[parameter.name]+":"+user_params[parameter.name]+"   "+str(len(parameter)))
    ...
    def check_parameter_rule(self, rule: str, src_arg: str, user_args: dict, nb_rows: dict):
        ...
        if is_conditional_rule:
            ...
            elif hasattr(j_rules, name):
                validator_class = getattr(j_rules, name)
            else:
